...
function Drawing(tool, points, 
   ...
   this.text = words;
   this.font = font;
   this.fE = fillingEnabled;
   this.sE = strokingEnabled;
};
...
function makeStrokeSizeBox(elementName, callback, defaultVal){
   var ssb = document.createElement('input')
   ssb.setAttribute('id', elementName)
   $(ssb).val(defaultVal)
   $(ssb).keyup(function(e){callback($(ssb).val())})
   ssb.setAttribute('style', 'height:22px; width:22px; float:left; margin:1px; border:1px solid gray; text-align:left')
   $('#canvas_toolbar').append(ssb)
}
...
function redraw(drawing){//Later implementation
   ...
   if (pts.length >= 2){
      ...
      BGctx.lineWidth = drawing.sSiz;
      //console.log("Making drawing: fill:{0}|{3}, stroke:{1}px {2}".format(BGctx.fillStyle, BGctx.strokeStyle, BGctx.lineWidth, drawing.fE));
      switch(tool){
         case "freehand":
            BGctx.beginPath();
         ...
         case "text":
            ...
            BGctx.strokeText(drawing.text, pts[1].x, pts[1].y);
            break;
         case "ellipse":
            clearCanvas(ctx);
      ...
      if(drawing.fE)BGctx.fill();
      if(drawing.sE)BGctx.stroke();
   }
};
