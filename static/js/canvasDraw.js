   ...
   if (pts.length >= 2){
      ...
      switch(tool){
         ...
         case "text":
            ...
            if(drawing.fE){BGctx.fillText(drawing.text, pts[1].x, pts[1].y);}
            if(drawing.sE){BGctx.strokeText(drawing.text, pts[1].x, pts[1].y);}
            break;
         case "ellipse":
            //clearCanvas(ctx);
      ...
      if(drawing.fE){BGctx.closePath(); BGctx.fill();}
...
function makeDatabaseEntry(drawing){
